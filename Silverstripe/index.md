...
Steps to setup a relationship for the `LandingPage` Model to have a `Service` type selection:
- A `ServiceType` DataObject which will be a new table that has a `has_many` relationship to a `LandingPage`
- Create a class for displaying a global menu for `ServiceTypes` called
- a `has_one` relationship from `LandingPage` to `ServiceType` and a radio button to allow admins to choose a service type
